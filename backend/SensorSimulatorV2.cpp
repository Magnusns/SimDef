...
private:
    ...
    double beregnVinkelMotBase(const Objekt& obj) {
        double dx = -obj.x;
        double dy = -obj.y;
        double baseVinkel = atan2(dy, dx) * 180.0 / M_PI;
        if (baseVinkel < 0) baseVinkel += 360;

        double diff = fabs(baseVinkel - obj.retning);
        if (diff > 180) diff = 360 - diff;
        return diff; // 0 = rett mot base
    }

    std::string vurderTrussel(const Objekt& obj) {
        double dist = std::sqrt(obj.x * obj.x + obj.y * obj.y);
        double vinkelDiff = beregnVinkelMotBase(obj);

        if (dist < 30 && vinkelDiff < 30 && obj.hastighet > 4.0) {
            return "trussel";
        } else if (obj.hastighet > 4.5 || (dist < 50 && vinkelDiff < 45)) {
            return "mistenkelig";
        }
        return "normal";
    }
